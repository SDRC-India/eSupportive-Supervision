...
import { IonicPage, NavController, Events, AlertController, Platform, MenuController } from 'ionic-angular';
...
export class EditFormPage {
  ...
  ngOnInit():void {
    ...
    this.events.subscribe(MessageProvider.EVENTS.SAVED_FORMS, (flag, data,message) => {      
     ...
     this.events.subscribe(MessageProvider.EVENTS.SAVED_FORM_DELETED, (flag, message) => {      
              if(flag){
 ...
 */
  delete(id : Date){
    ...
    let confirm = this.alertCtrl.create({enableBackdropDismiss:false});
        ...
        confirm.setMessage('Are you sure you want to delete this saved checklist?');
        ...
        confirm.addButton({
          ...
          handler: () => {
            temp.id = id;
            temp.dataService.deleteTxnData(MessageProvider.MENU.EDIT_SAVED_FORM, id, false, null);
            }
          });
        confirm.present();    
  }
  ...
  ngOnDestroy(): void{
    this.events.unsubscribe(MessageProvider.EVENTS.SAVED_FORM_DELETED);

    //We called the following line cause it will reflect the number of saved forms in home page.
    this.dataService.getSavedForms();
  }
