...
function reportController($scope, $http, $filter ,$timeout, $window,$compile) {
	...
	$scope.pageName = "Report > Improvement In Facility";
	$scope.activeMenu = "improvement";
	...
	$http.get("typeDetails").then(function(result) {
		...
		{
			if($scope.checklists[i].key==121)
				{
				$scope.selectedCheckList=$scope.checklists[i];
				break;
				}
		}
    ...
    */
	...
	$scope.selectBlock = function(block) {	
		...
		if($scope.selectedBlock.areaNId!=0)
			{
		$scope.facilityList = $filter('filter')($scope.location, {
			...
			{
			$scope.facilityList = $filter('filter')($scope.location, {
				areaId : $scope.selectedDist.areaId, level : 5
			});
			}
	};
	...
	$scope.getTableData = function(){
		...
		// get all table data
		$http.post("getImprovementInFacilityReport",reportInputDataModel).then(function(result) {
			$scope.tableFilterWord=undefined;
			$scope.reportTableData =[];
			if(result.data.length>0)
			{		$scope.facilities=$scope.facilityList.length
	...
	$scope.getFacilityData = function(rowData){
		...
		$scope.block=rowData['area'];
		$scope.facility=rowData['facility'];
		$scope.level=rowData['facilityType'];
		var reportInputDataModel=
		{
			checklistType:$scope.selectedCheckList.key,
			submisionIDs:rowData['submissionIds'],
		facilityId:rowData['facilityID']==undefined?0:rowData['facilityID']
			
		};
		$http.post(	'getImprovementInFacilitySubmissionReport',reportInputDataModel)
	...
	$scope.exportTableData = function(id, facility, block){
        ...
        var tab_text = "<h2>Improvement In Facility</h2><h3>Checklist: "+$scope.savedSelectedChecklist.value+"</h3>"+(facility?"<h3>Facility Type: "+$scope.level+"</h3>":"")+ (facility?"<h3>Facility: "+facility+", "+ block +"</h3>":"")+""+"<h3>Location: " + $scope.savedSelectedDistrict.name + ", " +$scope.savedSelectedState.name +"</h3>";
        if(id == "dataTable"){
        ...
        if(id=="dataTable"){
        	link.download = "improvement_in_facility_" + $scope.savedSelectedState.name +"_" + new Date().toLocaleDateString() + ".xls";
        }else{
        	link.download = "improvement_in_facility_" + $scope.savedSelectedState.name +"_" + $scope.facility + "_" + new Date().toLocaleDateString() + ".xls";
        }
