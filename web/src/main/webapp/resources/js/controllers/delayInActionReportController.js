...
function reportController($scope, $http, $filter ,$timeout, $window,$compile) {
	...
	$scope.pageName = "Report > Delay In Action Item";
	$scope.activeMenu = "delayactionitem";
    ...
    */
	...
	$scope.getTableData = function(){
		...
		// get all table data
		$http.post("getDelayedPlanofActionForDistrict",reportInputDataModel).then(function(result) {
	...
	$scope.getFacilityData = function(rowData){
		...
		$http.post('getDelayedPlanOfActionForAFacility?facilityId=' + rowData['facilityId']
	...
	$scope.exportTableData = function(id, facility, block){
        ...
        var tab_text = "<h2>Delay in Action Item</h2><h3>Checklist: "+$scope.savedSelectedChecklist.value+"</h3>"+(facility?"<h3>Facility: "+facility+", "+ block +"</h3>":"")+"<h3>Location: " + $scope.savedSelectedDistrict.name + ", " +$scope.savedSelectedState.name +"</h3>";
        if(id == "dataTable"){
        ...
        if(id=="dataTable"){
        	link.download = "delay_in_action_report_" + $scope.savedSelectedState.name +"_" + new Date().toLocaleDateString() + ".xls";
        }else{
        	link.download = "delay_in_action_report_" + $scope.savedSelectedState.name +"_" + $scope.facility + "_" + new Date().toLocaleDateString() + ".xls";
        }
