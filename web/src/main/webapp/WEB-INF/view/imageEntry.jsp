...
<script type="text/javascript" src="resources/js/controllers/imageEntryController.js"></script>
<style type="text/css">
...
<body ng-app="imageEntryApp" ng-controller="imageEntryController" ng-cloak>
		...
		<div class="container entry-content" >
				...
				<div class="container-fluid">
					<div class="col-md-12">
						<div class="col-md-4 text-xs-left text-md-right">
							<label for="textinput">Title of the Image <span class="mandatory_star">&#42;</span></label>
						</div>
						<div class="col-md-5">
							<input id="imageTitle" placeholder="Enter Image Title"
								ng-model="imageDetails.title"
			...
			<div class="row newsupadates-row">
				<div class="container-fluid">
					<div class="col-md-12">
						<div class="col-md-4 text-xs-left text-md-right">
							<label for="textinput" class="col-md-12 Img_label">Upload Image <span
								class="mandatory_star">&#42;</span></label>
						</div>
						<div class="col-md-5" enctype="multipart/form-data">
							<input type="file" class="file" id="uplodsal" accept="image/*"
								onchange="angular.element('body').scope().getUploadFile(this)"
								ng-model="salutationUpload" value="files[0]" />
							<button class="upload btn btn-primary input-lg uploadFilenWidth upload-image-entry"
								id="uploadsalfile1" type="button">UPLOAD</button>

							<img class="image-box-gallery" id="update-profile-image"
								ng-show="imageDetails.images" data-action="zoom"
								ng-src="{{imageDetails.images}}">
						</div>
					</div>
				</div>
			</div>
			...
			<div ng-show="imageGalleryList" class="row" style="margin-bottom: 20px;">
				<div class="col-md-12 news-image-padding"
					ng-show="imageGalleryList.length">
					<div
						class="col-md-6 col-sm-6 col-xs-12 text-md-left text-sm-left text-xs-center
						deg-show-result news-image-padding">
						<b>Showing
							{{(imageGalleryList|filter:tableFilterWord).length}} out of
							{{imageGalleryList.length}}</b>
					</div>
					<div
						class="col-md-6 col-sm-6 col-xs-12 text-md-right text-sm-right text-xs-center news-image-padding">
						...
						<div class="select-container text-center" style="margin: 0px !important;">
							<div class="input-group" style="margin: auto;">
								<input type="text" placeholder="Search the section below" id="searchDashboard"
									ng-change="imageGalleryData(newsTitle)"
								...
								<div class="input-group-btn" style="position: relative;">
									...
									style="padding: 9px 12px;!important;" 
										type="button" ng-click="search(newsTitle)">
			...
			<div class="row">
			<div class="table-responsive header-fixed-table image-table"
					style="width: 100%;" sdrc-table-header-fix
					tableuniqueclass="'image-table'"
					tabledata="imageGalleryList">
			<table items="tableData" show-filter="true" cellpadding="0"
						cellspacing="0" border="0" class="dataTable table image-bottom table-striped "
						id="dataTable" ng-show="imageGalleryList.length>0">
				<thead>
					<th>Title
				...
				<tbody>
					<tr dir-paginate="rowData in imageGalleryList | orderBy:filterType:sortReverse | filter:tableFilterWord |itemsPerPage: pageSize">
						<td class="pdsa-table" sortable="'{{rowData.column}}'">{{rowData.title}}</td>
						<td class="pdsa-table" sortable="'{{rowData.column}}'">
						<img class="image-box-gallery" id="update-profile-image" ng-src="{{rowData.images}}">
					    </td>
						<td class="pdsa-table" sortable="'{{rowData.column}}'">{{rowData.createdDate}}</td>
						<td class="pdsa-table"><button id="buttonreset" name="buttonreset" ng-click="editEarmark(rowData.imageId)"
					class="btn btn-info" type="button">Edit</button><button id="buttonreset" name="buttonreset" ng-click="deleteConfirmation(rowData.imageId)"
				...
				<div class="col-md-12 text-center no-result no-result-bottom"
					ng-show="(imageGalleryList|filter:tableFilterWord).length == 0">
					No Result Found
				</div>
				<!--Pagination -->
				...
				<div class="modal-dialog">
					...
					<div class="modal-content">
						<div class="modal-body text-center">
							...
							<span><button type="button" class="btn errorOk" id="uploadModal" ng-click="deleteImageGallery()" >Yes</button></span>
