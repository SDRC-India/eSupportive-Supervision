...
<html ng-app="monitoringReportApp">
<head>
...
<body ng-controller="monitoringReportController" ng-cloak>
	...
	<div id="mymain" class="container" style="margin-top: 150px;">
		<div class="row report-container">
			...
			<div class="col-md-12 col-sm-12 col-xs-12 text-md-right">
				<div
					class="col-md-3 col-md-offset-1 col-sm-6 col-xs-12 thematic-selection-container">
			...
			<div
				...
				<div class="col-md-3 col-md-offset-1 col-sm-6 col-xs-12 text-md-center thematic-selection-container">
					<div class="select-container text-center ipad-margin">
	...
	<div class="row" style="margin-top: 40px; margin-bottom: 70px;" ng-show="data.length">
				...
				<!-- <div class="table-responsive header-fixed-table plan-actual-table monitor-user-table table-height-all all-table-height" > -->
		<div class="table-responsive header-fixed-table monitor-user-table table-height-all all-table-height"
							style="width: 100%; overflow: auto;"
							tableuniqueclass="'monitor-user-table'"
							tabledata="data">
		<table items="tabledata" show-filter="true" cellpadding="0" 
					...
					<thead>
						<tr>
							...
							<th class="deg-mgmt-table-head" nowrap
								...
								<div class="sorting1" ng-click="order('registeredUsers')">
									<i class="fa fa-caret-up fa-lg asc" aria-hidden="true"
										ng-class="{'hiding': sortReverse == true || (sortType != 'registeredUsers' &&  sortReverse == false)}"></i>
									<i class="fa fa-caret-down fa-lg dsc" aria-hidden="true"
										ng-class="{'hiding': sortType == 'registeredUsers' &&  sortReverse == false}"></i>
								</div>
							</th>
							<th class="deg-mgmt-table-head" nowrap
								style="font-size: 14px; font-weight: bold; width: 20%; vertical-align: middle;"
								>No of users logged in 
								<div class="sorting1" ng-click="order('loggedInUsers')">
									<i class="fa fa-caret-up fa-lg asc" aria-hidden="true"
										ng-class="{'hiding': sortReverse == true || (sortType != 'loggedInUsers' &&  sortReverse == false)}"></i>
									<i class="fa fa-caret-down fa-lg dsc" aria-hidden="true"
										ng-class="{'hiding': sortType == 'loggedInUsers' &&  sortReverse == false}"></i>
								</div>
							</th>
						</tr>
					</thead>
					...
					<tbody id="myTable">
						<tr	ng-repeat="rowData in data | orderBy:filterType:sortReverse  | filter:tableFilterWord">
							<td class="pdsa-table" sortable="'{{rowData.column}}'">{{$index+1}}</td>
							<td class="pdsa-table" sortable="'{{rowData.column}}'">{{rowData.areaName}}</td>
							<td class="pdsa-table" sortable="'{{rowData.column}}'">{{rowData.registeredUsers}}</td>
							<td class="pdsa-table" sortable="'{{rowData.column}}'">{{rowData.loggedInUsers}}</td>
						</tr>
					</tbody>
				</table>
	</div>
	...
	<script type="text/javascript" src="resources/js/controllers/monitoringReportController.js"></script>
	...
	<script type="text/javascript">
		var app = angular.module("monitoringReportApp", []);
		var myAppConstructor = angular.module("monitoringReportApp");
		myAppConstructor.controller("monitoringReportController",
